cc.Class({
    ...
    check_update(delegate,mod){
        ...
        this.file_module = mod || 'Main'
        this.file_version = 'version.manifest'
        this.file_project = 'project.manifest'
        if(this.file_module != ''){
            this.file_version = this.file_module + '/version.manifest'
            this.file_project = this.file_module + '/project.manifest'
        }
        this.file_manifest = ''
        ...
        // else if(mod == '' || mod == 'Main') //大厅或单包，请确认你的包体内包含资源列表配置文件
        ...
        this.hotup.init(d)
        this.hotup.check_update('http://192.168.1.53/jf2',this.updir)
    },

    //更新前检测旧版本目录 和 本地缓存目录是否存在未完成的更新
    check_before(){
        let path = jsb.fileUtils.getWritablePath() + this.updir+ "/"+  this.file_module + '_temp'
        ...
        let pathDir = jsb.fileUtils.getWritablePath() + this.updir+ '_temp';
        console.log('check_before Directory:',pathDir)
        // let file = path + '/project.manifest.temp'
        if(jsb.fileUtils.isDirectoryExist(pathDir)){
            let removed = jsb.fileUtils.removeDirectory(pathDir)
            console.log('removeDirectory:',pathDir,removed)
        }

        //remove vsrsion files
        let versionFileTmp = jsb.fileUtils.getWritablePath() + this.updir + '_temp/version.manifest.tmp';
        console.log('check_before versionFileTmp:',versionFileTmp)
        if(jsb.fileUtils.isFileExist(versionFileTmp)){
            console.log('remove temp version file:'+jsb.fileUtils.removeFile(versionFileTmp));
        }
    },
    ...
    check_after(finish){
        ...
        if(jsb.fileUtils.isFileExist(file)){
            ...
            //         console.log('version file writeStringToFile',newfile,jsb.fileUtils.writeStringToFile(JSON.stringify(str),newfile));
            //     }
        ...
        if(jsb.fileUtils.isFileExist(file)){
            ...
            if(mod && mod == this.file_module){
                if(file == newfile)
                    ...
                    let fullPath = jsb.fileUtils.getFileDir(newfile);
                    let dirExit = jsb.fileUtils.isDirectoryExist(fullPath);
                    if(!dirExit){
                        let createDir = jsb.fileUtils.createDirectory(fullPath);
                        console.log("createDir:",fullPath,createDir);
                    }
                    let stringToFile = jsb.fileUtils.writeStringToFile(str,newfile);
                    console.log('project file writeStringToFile',newfile,stringToFile);
